...
%Less efficient than cell_forces_stress_vector due to not using vectorised 
global eta alpha s0 F N M E r_rec  t_rec T fixlist k;
dxdt = zeros(4*N,1);
...
for i = 1:M;
    ...
    ref_force = k*eta*((alpha*(norm(rho_ij)-s0)+(1-alpha)*(norm(rho_ij)-norm(r_av)))*rho_ij/norm(rho_ij));
